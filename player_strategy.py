...
class PlayerStrategy(Frame):
	...
	def create_widgets(self):
		...
		self.session = Label(self, text="Hours Per Session:")
		self.session.grid(row=4, column=0, sticky=W)
		self.session_entry = Entry(self, width=5)
		self.session_entry.insert(INSERT, "3")
		self.session_entry.grid(row=4, column=1)
		self.count_bet = Label(self, text="Count To Increase:")
		self.count_bet.grid(row=5, column=0, sticky=W)
		self.count_bet_entry = Entry(self, width=5)
		self.count_bet_entry.insert(INSERT, "2")
		self.count_bet_entry.grid(row=5, column=1)
		self.bet_inc = Label(self, text="Bet Increase:")
		self.bet_inc.grid(row=6, column=0, sticky=W)
		self.bet_inc_entry = Entry(self, width=5)
		self.bet_inc_entry.insert(INSERT, "2")
		self.bet_inc_entry.grid(row=6, column=1)
