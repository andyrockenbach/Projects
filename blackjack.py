...
class Blackjack(Frame):
	...
	def create_widgets(self):
		...
		self.player_strategy.grid(row=4, column=1)
	...
	def run_simulation(self):
		...
		card_table.set_table_style(self.rules.insurance.get(), self.rules.surrender.get(), float(self.rules.blackjack_payout.get()), int(self.rules.decks.get()))
		...
		num_bankrupt = float(bankrupt) / float(10)
